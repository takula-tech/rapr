...
- **Enhanced Performance**
  - Rapr is built with performance in mind with `RUST` language,
  providing low-latency and high-throughput capabilities for distributed applications.
...
- **Enhanced Actor Build Block**
  ...
  - `Actor Placement`:
    ...
    - a random host
  - `Actor Migration`: User can trigger migration programmatically from codes.
    For latency-sensitive applications, user can trigger live migration  
    without dropping requests or losing in-memory state, requiring no  
    read/write operations of database.
  - `Rebalance with custom strategy`: rebalance the actors when new host joins or  
    leaves the cluster (shutdown, crash or upgrade) based on predefined or use-defined strategy.
